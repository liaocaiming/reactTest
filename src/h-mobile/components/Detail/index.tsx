...
import { IProps, IRow } from "./interface";
          ...
          return (
            ...
            >
              <div
                className={helpers.reactClassNameJoin([
                  "detail-item-content",
                  isTwoRow ? "detail-item-flex" : "",
                ])}
              >
                <Toggle isShow={!!label}>
                  <span
                    className={`detail-item-label detail-item-label-${name}`}
                  >
                    {label}
                  </span>
                </Toggle>
  ...
  return (
    <div className="mb-detail">
      {rows.map((rowArr: IRow[], index: number) => {
        return (
          <div key={String(index)} className={`row-item row-item-${index}`}>
            {renderContent(rowArr)}
          </div>
        );
      })}
    </div>
